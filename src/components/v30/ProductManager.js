...
const ProductManager = ({ products, setProducts, categories, setCategories, className, canCreate, onLimitReached }) => {
    ...
    const addCategory = () => { if (newCatName && !categories.includes(newCatName)) { setCategories([...categories, newCatName]); setNewCatName(''); } };
    ...
    if (isEditing) return (
        <div className={`p-4 md:p-8 max-w-6xl mx-auto ${className} animate-fade-in app-tab overflow-y-auto h-full`}>
            ...
            <div className="grid md:grid-cols-12 gap-6">
                <div className="md:col-span-4 space-y-6">
                    <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-4 shadow-sm">
                        ...
                        <div><label className="text-[10px] font-black uppercase text-slate-400 mb-1 block">Categoría</label><select className="w-full p-4 border rounded-xl bg-slate-50 outline-none" value={currentProduct.category} onChange={e => setCurrentProduct({ ...currentProduct, category: e.target.value })}>{categories.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
    ...
    return (
        <div className={`p-4 md:p-8 max-w-7xl mx-auto ${className} animate-fade-in app-tab overflow-y-auto h-full`}>
