    ...
    return (
        <div ref={setNodeRef} style={style} className={`group bg-white dark:bg-slate-800 rounded-[24px] border border-slate-200/60 dark:border-slate-700/60 ${isDragging ? 'shadow-2xl scale-105 ring-4 ring-blue-500/20' : 'shadow-sm hover:shadow-xl hover:border-slate-300 dark:hover:border-slate-600'} overflow-hidden transition-all flex flex-col h-full relative`}>
            ...
            {filterCategory === 'Todas' && (
                ...
                >
                    <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-md rounded-full shadow-lg border border-slate-200 dark:border-slate-700 w-12 h-12 flex items-center justify-center hover:scale-105 active:scale-95 transition-transform">
            ...
            <div className="p-4 flex-1 flex flex-col bg-white dark:bg-slate-800 z-30">
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className="flex-1 flex items-center justify-center gap-2 min-h-[48px] bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-2xl font-semibold text-sm transition-all"
                    >
                        <Edit size={18} /> <span className="inline">Editar</span>
                    </button>

                    <button
                        onClick={(e) => { e.stopPropagation(); confirmDelete(p.id); }}
                        className="w-12 h-12 min-h-[48px] flex shrink-0 items-center justify-center bg-red-50 hover:bg-red-100 dark:bg-red-900/20 dark:hover:bg-red-900/40 text-red-600 dark:text-red-400 rounded-2xl transition-all"
                        title="Borrar"
                    >
                        <Trash size={18} />
                    </button>
...
const ProductManager = ({ products, setProducts, categories, setCategories, className, canCreate, onLimitReached }) => {
    ...
    const handleSave = () => { 
        if (!currentProduct.name) return toast("Nombre obligatorio", "error"); 
        const isNew = !products.some(p => p.id === currentProduct.id);
        try {
            posthog.capture(isNew ? 'product_added' : 'product_edited', {
                product_name: currentProduct.name,
                category: currentProduct.category,
                price_type: currentProduct.priceType
            });
        } catch (e) { console.error('PostHog error:', e); }
        setProducts(prev => { 
            const idx = prev.findIndex(p => p.id === currentProduct.id); 
            return idx >= 0 ? prev.map((x, i) => i === idx ? currentProduct : x) : [...prev, currentProduct]; 
        }); 
        setIsEditing(false); 
    ...
    if (isEditing) return (
        <div key="editor-mode" className={`p-4 md:p-8 max-w-3xl mx-auto ${className} animate-fade-in app-tab overflow-y-auto h-full`}>
            {/* ... content ... */}
            <div className="flex items-center justify-between gap-3 mb-5 sticky top-0 bg-slate-50/95 dark:bg-slate-900/95 backdrop-blur-sm z-20 py-3">
                <div className="flex items-center gap-2 min-w-0">
                    <button onClick={() => setIsEditing(false)} title="Cancelar" className="shrink-0 w-10 h-10 flex items-center justify-center rounded-xl text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors"><ArrowLeft size={20} /></button>
                    <div className="min-w-0">
                        <h2 className="text-lg md:text-2xl font-black text-slate-800 dark:text-slate-100 leading-tight truncate"><span className="md:hidden">Producto</span><span className="hidden md:inline">Editor de Producto</span></h2>
                        <p className="text-[11px] text-slate-400 dark:text-slate-500 font-medium hidden md:block">Configura precios y extras</p>
                    </div>
                </div>
                <button onClick={handleSave} className="shrink-0 px-4 md:px-6 py-2.5 min-h-[44px] rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold shadow-lg shadow-blue-500/20 flex items-center gap-2 transition-colors"><Save size={18} /> Guardar</button>
            </div>
            <div className="space-y-4 md:space-y-5">
                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 space-y-4 shadow-sm">
                        <div><label className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 mb-1 block">Nombre del Producto</label><input placeholder="Ej: Ventana corredera 2 hojas" className="w-full p-3 min-h-[48px] border dark:border-slate-700 rounded-xl font-bold text-lg bg-slate-50 dark:bg-slate-900 dark:text-slate-100 placeholder:font-medium placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:bg-white dark:focus:bg-slate-800 focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900 outline-none" value={currentProduct.name} onChange={e => setCurrentProduct({ ...currentProduct, name: e.target.value })} /></div>
                        <div>
                            <label className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 mb-1 block">Categoría</label>
                            <div className="flex gap-2">
                                <select className="w-full p-4 min-h-[48px] border dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-900 dark:text-slate-100 outline-none" value={currentProduct.category} onChange={e => setCurrentProduct({ ...currentProduct, category: e.target.value })}>
                                    {categories.map(c => <option key={c} value={c}>{c}</option>)}
                                </select>
                                <button onClick={() => setShowCatManager(!showCatManager)} className="bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-800/40 p-4 min-h-[48px] rounded-xl hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors" title="Nueva Categoría">
                                    <Plus size={20} />
                                </button>
                            </div>
                            {showCatManager && (
                                <div className="mt-2 flex gap-2 animate-fade-in-down">
                                    <input autoFocus placeholder="Nombre nueva categoría..." className="flex-1 p-3 min-h-[48px] border dark:border-slate-700 rounded-xl bg-white dark:bg-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-100 outline-none text-sm" value={newCatName} onChange={e => setNewCatName(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { addCategory(); setShowCatManager(false); } }} />
                                    <button onClick={() => { addCategory(); setShowCatManager(false); }} className="px-4 py-3 min-h-[48px] bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700">Añadir</button>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-xl border border-blue-100 dark:border-blue-800/40">
                            ...
                            <div className="flex gap-2">
                                <div className="flex rounded-lg bg-white dark:bg-slate-800 border dark:border-slate-700 overflow-hidden shrink-0">
                                    <button onClick={() => setCurrentProduct({ ...currentProduct, marginType: 'percent' })} className={`px-4 py-3 min-h-[48px] text-sm font-bold transition-colors ${currentProduct.marginType === 'percent' ? 'bg-blue-600 text-white' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>%</button>
                                    <button onClick={() => setCurrentProduct({ ...currentProduct, marginType: 'fixed' })} className={`px-4 py-3 min-h-[48px] text-sm font-bold transition-colors ${currentProduct.marginType === 'fixed' ? 'bg-blue-600 text-white' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>€</button>
                                </div>
                                <input type="number" className="w-full p-3 min-h-[48px] text-lg outline-none bg-white dark:bg-slate-800 dark:text-slate-100 border dark:border-slate-700 rounded-lg" placeholder="0" value={currentProduct.marginValue} onChange={e => setCurrentProduct({ ...currentProduct, marginValue: e.target.value })} />
                            </div>
                            <p className="text-[10px] text-blue-400 dark:text-blue-500 mt-1 italic">Se añade al coste base + extras</p>
                        </div>
                    </div>
                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-4 flex flex-col min-h-[320px]"><h3 className="font-bold text-lg mb-4 flex items-center gap-2 dark:text-slate-100"><div className="p-1 bg-blue-100 dark:bg-blue-900/30 rounded text-blue-600 dark:text-blue-400"><Wand size={16} /></div> Configuración de Precios</h3>{currentProduct.priceType === 'unit' ? <div className="flex-1 flex items-center justify-center flex-col"><label className="text-sm font-bold text-slate-400 dark:text-slate-500 mb-2">PRECIO BASE UNITARIO</label><div className="flex items-center w-full justify-center px-2"><span className="text-3xl md:text-4xl font-bold text-slate-300 dark:text-slate-600 mr-2 shrink-0">€</span><input type="number" placeholder="0" className="text-5xl md:text-6xl font-black border-b-2 border-slate-200 dark:border-slate-600 w-full max-w-[240px] min-w-0 outline-none text-center focus:border-blue-500 transition-colors bg-transparent dark:text-slate-100" value={currentProduct.unitPrice} onChange={e => setCurrentProduct({ ...currentProduct, unitPrice: e.target.value })} /></div></div> : (['simple_area', 'simple_linear'].includes(currentProduct.priceType) ? <div className="flex-1 flex items-center justify-center flex-col"><label className="text-sm font-bold text-slate-400 dark:text-slate-500 mb-2">PRECIO BASE POR {currentProduct.priceType === 'simple_area' ? 'METRO CUADRADO (m²)' : 'METRO LINEAL (ml)'}</label><div className="flex items-center w-full justify-center px-2"><span className="text-3xl md:text-4xl font-bold text-slate-300 dark:text-slate-600 mr-2 shrink-0">€</span><input type="number" placeholder="0" className="text-5xl md:text-6xl font-black border-b-2 border-slate-200 dark:border-slate-600 w-full max-w-[240px] min-w-0 outline-none text-center focus:border-blue-500 transition-colors bg-transparent dark:text-slate-100" value={currentProduct.unitPrice} onChange={e => setCurrentProduct({ ...currentProduct, unitPrice: e.target.value })} /></div></div> : <MatrixEditor matrix={currentProduct.matrix} onChange={m => setCurrentProduct({ ...currentProduct, matrix: m })} />)}</div>
                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm"><div className="flex items-center gap-2 mb-2"><Wand size={16} className="text-purple-500" /><h3 className="font-bold text-sm dark:text-slate-100">Extras y Acabados</h3></div>
                        <div className="space-y-2 mb-4 max-h-40 overflow-y-auto pr-1 custom-scrollbar">{currentProduct.extras.map(e => (<div key={e.id} className="flex justify-between items-center p-2 bg-white dark:bg-slate-700 border dark:border-slate-600 rounded-lg shadow-sm text-sm gap-2"><div className="flex flex-col min-w-0 flex-1 pl-2"><span className="font-medium dark:text-slate-200 truncate">{e.name}</span><span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400">{e.type === 'selection' ? 'Lista de opciones' : `+${e.value}${e.type === 'percent' ? '%' : (e.type === 'linear' ? ' €/ml' : (e.type === 'area' ? ' €/m²' : ' €'))}`}</span></div><div className="flex gap-1 shrink-0"><button onClick={() => { setNewExtra({ name: e.name, type: e.type, value: e.value }); setTempOptionsList(e.optionsList || []); setEditingExtraId(e.id) }} className="text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 p-2 min-h-[44px] min-w-[44px] flex justify-center items-center rounded-lg hover:bg-slate-100 dark:hover:bg-slate-600"><Edit size={16} /></button><button onClick={() => setCurrentProduct({ ...currentProduct, extras: currentProduct.extras.filter(ex => ex.id !== e.id) })} className="text-slate-400 hover:text-red-600 p-2 min-h-[44px] min-w-[44px] flex justify-center items-center rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20"><Trash size={16} /></button></div></div>))}</div>
                        <div className="p-3 bg-white dark:bg-slate-700/50 rounded-xl border border-blue-100 dark:border-slate-600 shadow-sm overflow-hidden"><div className="flex flex-col gap-2"><input placeholder="Nombre del extra" className="w-full p-3 min-h-[48px] border dark:border-slate-600 rounded-lg text-sm bg-slate-50 dark:bg-slate-800 dark:text-slate-100 focus:bg-white dark:focus:bg-slate-700" value={newExtra.name} onChange={e => setNewExtra({ ...newExtra, name: e.target.value })} /><div className="flex gap-2 w-full"><select className="shrink-0 w-[120px] p-3 min-h-[48px] border dark:border-slate-600 rounded-lg text-sm bg-slate-50 dark:bg-slate-800 dark:text-slate-200" value={newExtra.type} onChange={e => setNewExtra({ ...newExtra, type: e.target.value })}><option value="fixed">Precio (+€)</option><option value="percent">Porcen. (+%)</option><option value="linear">Por Metro (ml)</option><option value="area">Por m²</option><option value="selection">Lista Op.</option></select>{newExtra.type !== 'selection' && <input type="number" placeholder="0" className="min-w-0 flex-1 p-3 min-h-[48px] border dark:border-slate-600 rounded-lg text-sm bg-slate-50 dark:bg-slate-800 dark:text-slate-100" value={newExtra.value} onChange={e => setNewExtra({ ...newExtra, value: e.target.value })} />}</div>{newExtra.type === 'selection' && <div className="bg-slate-50 dark:bg-slate-800 p-2 rounded border dark:border-slate-600"><div className="grid grid-cols-12 md:flex gap-2 mb-2"><input placeholder="Opción" className="col-span-12 md:flex-1 p-3 min-h-[48px] text-sm border dark:border-slate-600 rounded-lg dark:bg-slate-700 dark:text-slate-100" value={newOption.name} onChange={e => setNewOption({ ...newOption, name: e.target.value })} /><input type="number" placeholder="Val" className="col-span-4 md:w-20 p-3 min-h-[48px] text-sm border dark:border-slate-600 rounded-lg dark:bg-slate-700 dark:text-slate-100" value={newOption.value} onChange={e => setNewOption({ ...newOption, value: e.target.value })} /><select className="col-span-5 md:w-28 p-3 min-h-[48px] text-sm border dark:border-slate-600 rounded-lg dark:bg-slate-700 dark:text-slate-100" value={newOption.type} onChange={e => setNewOption({ ...newOption, type: e.target.value })}><option value="fixed">€</option><option value="percent">%</option><option value="linear">€/ml</option><option value="area">€/m²</option></select><button onClick={() => { if (!newOption.name) return; setTempOptionsList([...tempOptionsList, { ...newOption }]); setNewOption({ name: '', value: 0, type: 'fixed' }) }} className="col-span-3 bg-blue-600 min-h-[48px] text-white rounded-lg flex items-center justify-center font-bold hover:bg-blue-700 transition-colors">+</button></div><div className="max-h-32 overflow-y-auto space-y-1 pr-1 custom-scrollbar">
                                ...
                                <div key={i} className="text-xs bg-white dark:bg-slate-700 p-1.5 border dark:border-slate-600 rounded-lg flex justify-between items-center group hover:border-blue-200 dark:hover:border-blue-600 transition-colors">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div className="flex gap-0.5 border-l dark:border-slate-600 pl-1">
                                            <button title="Editar" onClick={() => { setNewOption({ ...o }); setTempOptionsList(tempOptionsList.filter((_, idx) => idx !== i)); }} className="p-2 min-h-[44px] min-w-[44px] flex justify-center items-center text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded">
                                                <Edit size={16} />
                                            </button>
                                            <button title="Borrar" onClick={() => setTempOptionsList(tempOptionsList.filter((_, idx) => idx !== i))} className="p-2 min-h-[44px] min-w-[44px] flex justify-center items-center text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded">
                                                <Trash size={16} />
                                            </button>
                                        </div>
                                    </div>
