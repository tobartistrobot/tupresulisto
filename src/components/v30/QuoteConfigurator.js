...
const CartSummaryItem = React.memo(({ item, idx, onRemove, onUpdateQty, onMove }) => (
    <div className="p-3 md:p-4 border border-slate-200 dark:border-slate-700 rounded-xl md:rounded-2xl bg-white dark:bg-slate-800 shadow-sm hover:shadow-xl dark:hover:shadow-black/30 transition-all duration-300 mb-3 animate-fade-in flex flex-col gap-3">
        <div className="flex gap-3 md:gap-4">
            <div className="w-14 h-14 md:w-16 md:h-16 bg-slate-50 dark:bg-slate-700 rounded-lg md:rounded-xl flex items-center justify-center shrink-0 border border-slate-100 dark:border-slate-600">
                {item.product.image ? <img src={item.product.image} className="w-full h-full object-cover rounded-lg md:rounded-xl" /> : <Box className="text-slate-300 dark:text-slate-500" />}
            </div>
            <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start gap-2">
                    <p className="font-black text-sm md:text-base text-slate-900 dark:text-slate-100 leading-tight mb-1 truncate">{item.product.name}</p>
                    <button onClick={() => onRemove(item.id)} className="w-8 h-8 shrink-0 flex items-center justify-center text-red-500 hover:text-red-700 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/40 rounded-lg transition-colors border border-red-100 dark:border-red-900/30">
                        <Trash size={14} />
                    </button>
                </div>
                <p className="text-[11px] md:text-xs text-slate-500 dark:text-slate-400 font-medium line-clamp-2">{item.width}x{item.height} {item.locationLabel && `· ${item.locationLabel}`}</p>
                {item.selectedExtras?.length > 0 && (
                    <div className="mt-1.5 md:mt-2 flex flex-wrap gap-1">
                        {item.selectedExtras.map((e, i) => (
                            <span key={i} className="inline-flex items-center text-[9px] md:text-[10px] bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-600">
                                {e.qty > 1 && <b className="text-blue-600 dark:text-blue-400 mr-1">{e.qty}x</b>}{e.name}
                            </span>
                        ))}
                    </div>
                )}
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-100 dark:border-slate-700/50 mt-1 gap-2">
            <div className="flex items-center gap-1 bg-slate-50 dark:bg-slate-700 rounded-lg p-1 border border-slate-200 dark:border-slate-600 shrink-0">
                <button onClick={() => onUpdateQty(item.id, -1)} className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-600 rounded-md transition-colors text-slate-600 dark:text-slate-300 font-bold shadow-sm">-</button>
                <span className="text-sm font-bold w-6 md:w-10 text-center text-slate-700 dark:text-slate-200">{item.quantity}</span>
                <button onClick={() => onUpdateQty(item.id, 1)} className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-600 rounded-md transition-colors text-slate-600 dark:text-slate-300 font-bold shadow-sm">+</button>
            </div>
            
            <div className="flex flex-col items-end min-w-0">
                <b className="text-lg md:text-xl text-blue-900 dark:text-blue-300 font-black tracking-tight truncate max-w-full">{formatCurrency(item.price)}</b>
            </div>
        </div>
    </div>
));
    ...
    return (
        <div className={`flex flex-col md:flex-row h-full overflow-hidden ${className} app-tab`}>
            ...
            <div className={`${mobileTab === 'products' ? 'block' : 'hidden'} md:block w-full md:w-2/3 p-3 md:p-8 overflow-y-auto bg-slate-50 dark:bg-slate-950`}>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl md:rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-0 mb-6 md:mb-8 overflow-hidden relative z-20 w-full">
                    <div className="bg-slate-50 dark:bg-slate-900/50 p-3 border-b dark:border-slate-700 rounded-t-xl flex justify-between items-center"><span className="text-[10px] md:text-xs font-bold uppercase text-slate-500 dark:text-slate-400 tracking-wider ml-2">Datos del Cliente</span><button onClick={() => setShowClientSearch(!showClientSearch)} className="py-1.5 px-3 text-xs flex items-center gap-2 bg-white dark:bg-slate-700 border dark:border-slate-600 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-600 text-slate-600 dark:text-slate-300 font-bold"><Search size={14} /> Buscar</button></div>
                        ...
                        <div className="flex flex-col md:flex-row">
                            ...
                            <div className="flex-1 p-4 md:p-6 space-y-5 md:space-y-6">
                                ...
                                <div className="flex flex-col md:flex-row justify-between md:items-center pt-5 md:pt-6 border-t dark:border-slate-700/50 gap-4 mt-auto">
                                    ...
                                    <button onClick={addToQuote} disabled={!calculatedPrice} className={`btn-primary w-full md:w-auto px-6 md:px-8 py-3.5 md:py-4 text-base md:text-lg font-black uppercase tracking-widest rounded-xl md:rounded-2xl shadow-[0_4px_20px_rgba(37,99,235,0.4)] transition-all duration-300 min-h-[50px] md:min-h-[56px] ${calculatedPrice ? 'opacity-100 hover:scale-[1.02]' : 'opacity-50 grayscale cursor-not-allowed'}`}>
