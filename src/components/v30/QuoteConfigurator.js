...
const QuoteConfigurator = ({ products, categories, config, cart, setCart, onSave, onReset, initialData, clientsDb, className, isPro, onUpgrade }) => {
    ...
    return (
        <div className={`flex flex-col md:flex-row h-full overflow-hidden ${className} app-tab`}>
            <div className="md:hidden flex bg-white dark:bg-slate-900 border-b dark:border-slate-700 shadow-sm z-30 shrink-0"><button onClick={() => setMobileTab('products')} className={`flex-1 py-3 text-sm font-bold border-b-2 transition-colors ${mobileTab === 'products' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 dark:text-slate-400'}`}>Catálogo</button><button onClick={() => setMobileTab('cart')} className={`flex-1 py-3 text-sm font-bold border-b-2 flex items-center justify-center gap-2 transition-colors ${mobileTab === 'cart' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 dark:text-slate-400'}`}>Presupuesto <span className="bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300 text-[10px] px-2 py-0.5 rounded-full font-black">{cart.length}</span></button></div>
            <div className={`${mobileTab === 'products' ? 'block' : 'hidden'} md:block w-full md:w-2/3 p-4 md:p-6 overflow-y-auto bg-slate-50/50 dark:bg-slate-900/50`}>
                ...
                {!selectedProduct ? (
                    <div className="animate-fade-in">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">{products.filter(p => (filterCategory === 'Todas' || p.category === filterCategory) && p.name.toLowerCase().includes(filterTerm.toLowerCase())).map((p) => {
                            ...
                            return (
                                <div key={p.id} onClick={() => isLocked ? onUpgrade("Este producto es parte de tu historial PRO. Reactiva tu suscripción para desbloquearlo y usarlo.") : setSelectedProduct(p)} className={`group bg-white dark:bg-slate-800 rounded-2xl border ${isLocked ? 'border-slate-200 dark:border-slate-700 opacity-50 cursor-pointer grayscale' : 'border-slate-100 dark:border-slate-700 hover:border-blue-200 dark:hover:border-blue-600 hover:shadow-2xl dark:hover:shadow-black/40 hover:-translate-y-2 cursor-pointer'} overflow-hidden transition-all duration-300 relative`}>
                                    <div className="aspect-square bg-slate-50 dark:bg-slate-700 relative w-full overflow-hidden">
                                        {p.image ? <img src={p.image} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" /> : <div className="flex h-full items-center justify-center text-slate-300 dark:text-slate-500"><Box size={40} /></div>}
                                        {isLocked ? (
                                            <div className="absolute inset-0 bg-black/5 flex items-center justify-center">
                                                <div className="bg-white/90 dark:bg-slate-800/90 p-3 rounded-full shadow-lg backdrop-blur-sm transform group-hover:scale-110 transition-transform">
                                    ...
                                    <div className="p-4">
                                        <h3 className="font-black text-base text-slate-900 dark:text-slate-100 leading-tight mb-2">{p.name}</h3>
                                        <div className="flex justify-between items-center">
                                            <p className="text-[11px] text-slate-600 dark:text-slate-400 uppercase tracking-wider font-extrabold bg-gradient-to-r from-slate-100 dark:from-slate-700 to-slate-50 dark:to-slate-700/50 w-fit px-2.5 py-1 rounded-lg border border-slate-200 dark:border-slate-600">{p.category}</p>
                                            {isLocked && <span className="text-[10px] font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 px-2 py-0.5 rounded border border-amber-100 dark:border-amber-800/40">PRO</span>}
                                        </div>
                                    </div>
                                </div>
                            );
