...
const QuoteConfigurator = ({ products, categories, config, cart, setCart, onSave, onReset, initialData, clientsDb, className }) => {
    ...
    if (viewMode === 'print') return (
        <div className={`fixed inset-0 z-[100] bg-slate-100 flex flex-col h-safe-screen w-full fixed-print-view smooth-scroll overflow-auto`}>
            ...
            <div className="flex-1 overflow-auto p-4 md:p-8 flex justify-center items-start print:p-0 print:block">
            ...
            <div className="shrink-0 bg-white/80 backdrop-blur-md border-t flex gap-4 justify-center no-print z-50 p-4 shadow-2xl"><button onClick={() => setViewMode('edit')} className="px-4 py-2 rounded-lg border bg-white shadow-sm hover:bg-slate-50 font-bold text-slate-600">Seguir Editando</button><button onClick={() => window.print()} style={{ backgroundColor: config.color }} className="px-8 py-2 rounded-lg text-white font-bold shadow-xl flex items-center gap-2"><Printer size={18} /> Imprimir PDF</button><button onClick={downloadWord} className="px-4 py-2 rounded-lg border bg-white shadow-sm hover:bg-slate-50 font-bold text-slate-600 flex items-center gap-2"><FileText size={18} /> Word</button></div>
        </div>
    );
            ...
            <div className={`${mobileTab === 'products' ? 'block' : 'hidden'} md:block w-full md:w-2/3 p-4 md:p-6 overflow-y-auto bg-slate-50/50`}>
                ...
                <div className="bg-white rounded-xl border shadow-sm p-0 mb-6 overflow-visible relative z-20">
                    ...
                    <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="relative group"><label className="absolute -top-2 left-2 bg-white px-1 text-[10px] font-bold text-blue-500">Nombre</label><input className="w-full p-2.5 border rounded-lg text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" value={client.name} onChange={e => setClient({ ...client, name: e.target.value })} /></div>
                        <div className="relative group"><label className="absolute -top-2 left-2 bg-white px-1 text-[10px] font-bold text-slate-400">Teléfono</label><input className="w-full p-2.5 border rounded-lg text-sm outline-none focus:border-blue-500 transition-all" value={client.phone} onChange={e => setClient({ ...client, phone: e.target.value })} /></div>
                        <div className="relative group"><label className="absolute -top-2 left-2 bg-white px-1 text-[10px] font-bold text-slate-400">Email</label><input className="w-full p-2.5 border rounded-lg text-sm outline-none focus:border-blue-500 transition-all" value={client.email} onChange={e => setClient({ ...client, email: e.target.value })} /></div>
                        <div className="relative group"><label className="absolute -top-2 left-2 bg-white px-1 text-[10px] font-bold text-slate-400">Origen</label><select className="w-full p-2.5 border rounded-lg text-sm outline-none focus:border-blue-500 transition-all bg-white" value={client.source} onChange={e => setClient({ ...client, source: e.target.value })}><option value="">¿Cómo nos conoció?</option><option value="Recomendación">Recomendación</option><option value="Web">Web / Buscador</option><option value="Google">Google Maps</option><option value="Publicidad">Publicidad / Redes</option><option value="Tienda">Pasaba por tienda</option><option value="Otro">Otro</option></select></div>
                        <div className="relative group md:col-span-2"><label className="absolute -top-2 left-2 bg-white px-1 text-[10px] font-bold text-slate-400">Dirección</label><input className="w-full p-2.5 border rounded-lg text-sm outline-none focus:border-blue-500 transition-all" value={client.address} onChange={e => setClient({ ...client, address: e.target.value })} /></div>
                    </div>
                    {showClientSearch && <div className="absolute top-12 left-2 right-2 bg-white border shadow-2xl z-30 rounded-xl max-h-60 overflow-hidden flex flex-col animate-fade-in"><div className="p-2 border-b bg-slate-50"><input autoFocus placeholder="Filtrar por nombre o teléfono..." className="w-full p-2 text-xs border rounded bg-white outline-none focus:border-blue-500" value={clientSearchTerm} onChange={e => setClientSearchTerm(e.target.value)} /></div><div className="overflow-y-auto">{filteredClients.length === 0 && <p className="text-xs p-4 text-center text-slate-400">No hay clientes recientes.</p>}{filteredClients.map((c, i) => (<div key={i} onClick={() => { setClient({ ...c }); setShowClientSearch(false); setClientSearchTerm(''); }} className="p-3 hover:bg-blue-50 cursor-pointer border-b last:border-0 text-sm flex justify-between group"><span className="font-bold group-hover:text-blue-700">{c.name}</span><span className="text-slate-500">{c.phone}</span></div>))}</div></div>}
                </div>
                {!selectedProduct ? (
